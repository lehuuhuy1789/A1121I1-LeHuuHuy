...
where (year(hd.ngay_lam_hd)>=2020 and year(hd.ngay_lam_hd)<=2021)
...
where year(ngay_lam_hd)>=2019 and year(ngay_lam_hd)<=2021
group by ma_nhan_vien;

delete from nhan_vien_temp1
where ma_nhan_vien not in (select * from nhan_vien_lam_hd_views);
...
select kh.ma_khach_hang,kh.ho_ten,lk.ten_loai_khach,hd.so_hop_dong,dv.ten_dich_vu,hd.ngay_lam_hd, hd.ngay_ket_thuc, dv.chi_phi_thue+sum(hdct.so_luong)*dvdk.gia as tong_tien, lk.ma_loai_khach
...
select kh.ma_khach_hang,hd.ngay_lam_hd from hop_dong as hd join khach_hang as kh on hd.ma_khach_hang=kh.ma_khach_hang
...
create temporary table cap_nhat_gia_dich_vu_di_kem_temp(select dvdk.ma_dich_vu_di_kem, dvdk.ten_dich_vu_di_kem, dvdk.gia, sum(hdct.so_luong) as so_lan_su_dung, hd.ngay_lam_hd
from (dich_vu_di_kem as dvdk join hop_dong_chi_tiet as hdct on dvdk.ma_dich_vu_di_kem=hdct.ma_dich_vu_di_kem) join hop_dong as hd on hdct.ma_hop_dong=hd.so_hop_dong
group by dvdk.ma_dich_vu_di_kem);
